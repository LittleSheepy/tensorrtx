...
namespace ObjDet {
	...
	void Faster_RCNN::BuildRcnnModel(unsigned int maxBatchSize, IHostMemory** modelStream, const std::string& wtsfile,
	...
	void Faster_RCNN::serialize(std::string& wtsFile, std::string& engineFile) {
		if (wtsFile.empty()) { return; }
	...
	int Faster_RCNN::LoadEngine(std::string& engineFile) {
	...
	std::vector<FasterRCNN::Detection> Faster_RCNN::predict(cv::Mat& img) {
		// prepare input data
		...
		doInference(*m_context, m_stream, buffers, data, outputs);
		...
		std::vector<FasterRCNN::Detection> res;
